...
pub async fn list_pages_subscript(
    ...
    let filter_and = script_data
        .all_tags
        .iter()
        .fold("".to_string(), crom_recursive_query_builder("and"));

    let filter_or = script_data
        .one_of_tags
        .iter()
        .fold("".to_string(), crom_recursive_query_builder("or"));
    ...
    pages(
        global_data.verbose,
        global_data.site.as_ref().unwrap(),
        filter,
        script_data.author.as_ref(),
        info.to_string(),
...
fn _gciq_rec_fold(mut acc: Vec<QueryTree>, item: Vec<&str>) -> Vec<QueryTree> {
    match item.len() {
        ...
        1 => {
            acc.push(QueryTree::Node(item.first().unwrap().to_string()));
        }
        _ => {
            if acc.iter().any(|node| {
                if let QueryTree::MotherNode(str, _) = node {
                    str == item.first().unwrap()
                } else {
                    false
                }
            }) {
                ...
                acc.push(QueryTree::MotherNode(
                    item.first().unwrap().to_string(),
                    _gciq_rec_fold(Vec::new(), item[1..].to_vec()),
                ));
            }
        }
    }
            ...
            if filters.is_empty() {
                ...
                filters
                    ...
                    .find(|filter| match filter {
                        QueryTree::MotherNode(filter_str, _) | QueryTree::Node(filter_str) => {
                            str == *filter_str
                        }
                        _ => false,
                    })
...
pub async fn list_pages(mut script_data: Cli) {
    ...
    if params.txm {
        ...
        if (!params.source_contains.is_empty() || params.gather_fragments_sources)
            && !params.info.contains(&source_str)
        {
